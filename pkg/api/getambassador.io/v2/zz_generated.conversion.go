...
func autoConvert_v2_HttpHealthCheck_To_v3alpha1_HttpHealthCheck(in *HttpHealthCheck, out *v3alpha1.HttpHealthCheck, s conversion.Scope) error {
	...
	if true {
		...
		if *in == nil {
			...
			*out = new([]v3alpha1.StatusRange)
			in, out := *in, *out
			if *in == nil {
				*out = nil
			} else {
				*out = make([]v3alpha1.StatusRange, len(*in))
				for i := range *in {
					in, out := &(*in)[i], &(*out)[i]
					if err := Convert_v2_StatusRange_To_v3alpha1_StatusRange(in, out, s); err != nil {
						return err
					}
				}
			}
			...
			*out = new([]StatusRange)
			...
			if *in == nil {
				...
				*out = make([]StatusRange, len(*in))
				for i := range *in {
					in, out := &(*in)[i], &(*out)[i]
					if err := Convert_v3alpha1_StatusRange_To_v2_StatusRange(in, out, s); err != nil {
						return err
					}
				}
			}
